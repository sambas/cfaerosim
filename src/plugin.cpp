...
extern "C" int __stdcall DllMain(HINSTANCE hinstDLL, quint32 fdwReason, void *)
...
SIM_DLL_EXPORT void AeroSIMRC_Plugin_Init(pluginInit *p)
{
    ...
    sndr = new FC_Serial(ini->getInputMap(), ini->getOutputMap(), ini->isFromTX());
    sndr->init(ini->serialPort());
              ...
              pluginToSim *pts)
{
    ...
    debugInfo.append(
        ...
        .arg(stp->latitude, 5, 'f', 7)
        .arg(stp->longitude, 5, 'f', 7)
        .arg(stp->AGL, 5, 'f', 2)
        .arg(stp->heading * RAD2DEG, 5, 'f', 2)
        .arg(stp->pitch * RAD2DEG, 5, 'f', 2)
        .arg(stp->roll * RAD2DEG, 5, 'f', 2)
        .arg(stp->accelX/9.810*-1000, 5, 'f', 2)
        .arg(stp->accelY/9.810*-1000, 5, 'f', 2)
        .arg(stp->accelZ/9.810*1000+1000, 5, 'f', 2)
